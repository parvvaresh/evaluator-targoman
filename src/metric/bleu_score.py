    ...
    def bleu_score(self, refs, pred):
        ...
        for n in range(0, self.max_n_gram):
            clipped_count = self._clipped_count(
                refs_ngram_count[n], pred_ngram_count[n]
            )
        ...
        return math.exp(db) * self._brevity_penalty(
            self._get_closest_ref_lenght(refs_lenght, pred_lenght), pred_lenght
        )
