...
class nist:
    ...
    def nist_score(self, refs, pred):
        ...
        refs_ngrams = [
            [ngrams(ref, n) for n in range(1, self.number_n_grams + 1)] for ref in refs
        ]
            ...
            for num_gram in range(0, self.number_n_grams):
                ...
                for element in count_ngrams:
                    refs_ngrams_freq[element] = (
                        ...
                        if element in refs_ngrams_freq
                        else 1
                    )
        ...
        for n in range(0, self.number_n_grams):
            ...
            for index_ref in range(0, len(refs)):
                ...
                numerator = sum(
                    [info[n_gram] * count for n_gram, count in overlaps_ngram.items()]
                )
        ...
        return sum(nist_score) * self._length_penalty(
            (ref_lenght / self.number_n_grams), pred_lenght
        )
    ...
    def _info(self, refs_ngrams_freq, refs_total_words):
        ...
        for grams_1n in refs_ngrams_freq:
            ...
            occurrence = (
                refs_ngrams_freq[grams_1m]
                if grams_1m and grams_1m in refs_ngrams_freq
                else refs_total_words
            )
